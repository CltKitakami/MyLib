...
private:
	...
	static void ff(uint32_t &a, uint32_t b, uint32_t c,
		uint32_t d, uint32_t m, uint32_t s, uint32_t k)
	{ a = leftRotate(a+ f(b,c,d) + m + k, s) + b; }

	static void gg(uint32_t &a, uint32_t b, uint32_t c,
		uint32_t d, uint32_t m, uint32_t s, uint32_t k)
	{ a = leftRotate(a + g(b,c,d) + m + k, s) + b; }

	static void hh(uint32_t &a, uint32_t b, uint32_t c,
		uint32_t d, uint32_t m, uint32_t s, uint32_t k)
	{ a = leftRotate(a + h(b,c,d) + m + k, s) + b; }

	static void ii(uint32_t &a, uint32_t b, uint32_t c,
		uint32_t d, uint32_t m, uint32_t s, uint32_t k)
